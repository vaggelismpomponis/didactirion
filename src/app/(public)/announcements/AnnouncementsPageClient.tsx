...
type AnnouncementsHeaderContent = typeof defaultAnnouncementsHeaderContent;
type Post = {
  id: string;
  title: string;
  slug: string;
...
export function AnnouncementsPageClient({
  ...
  posts: Post[];
  initialContent: AnnouncementsHeaderContent;
}) {
  const [content, setContent] = React.useState<AnnouncementsHeaderContent>(initialContent);

  const handlePreviewUpdate = React.useCallback((override: Record<string, any>) => {
    setContent(mergeContent(defaultAnnouncementsHeaderContent, override) as AnnouncementsHeaderContent);
  }, []);

  return (
    <div className="flex flex-col">
      <PreviewListener pageKey="announcements-header" onContentUpdate={handlePreviewUpdate} />
            ...
            <div className="relative rounded-3xl overflow-hidden">
              ...
              <div className="relative p-7 text-white space-y-4">
                ...
                <Button asChild className="w-full bg-white text-primary hover:bg-white/90 font-bold">
                  <Link href="/contact">
                    <Editable id="cta_button">{content.cta_button}</Editable>
                  </Link>
                </Button>
              </div>
            </div>
