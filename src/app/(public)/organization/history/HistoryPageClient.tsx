            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {pillars.map((item, i) => (
                <div key={i} className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm hover:shadow-md hover:border-primary/30 hover:bg-primary/[0.01] transition-all duration-300 group">
                  <h4 className="font-bold text-slate-900 mb-2 group-hover:text-primary transition-colors text-base">
                    <Editable id={`pillars_${i}_title`}>{item.title}</Editable>
                  </h4>
                  <p className="text-sm text-slate-600 leading-relaxed">
                    <Editable id={`pillars_${i}_desc`}>{item.desc}</Editable>
                  </p>
                </div>
              ))}
            </div>
